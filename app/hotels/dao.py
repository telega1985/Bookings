...
class HotelDAO(BaseDAO):
    ...
    @classmethod
    async def get_list_hotels(cls, session: AsyncSession):
        query = select(cls.model)
        result = await session.execute(query)
        return result.scalars().all()

    @classmethod
    async def find_all_hotels(cls, session: AsyncSession, location: str, date_from: date, date_to: date):
        """
        ...
        ON booked_hotels.hotel_id = hotels.id
